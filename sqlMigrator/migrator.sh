#!/bin/bash
docker exec $1 psql -U admin -d postgres -c "CREATE DATABASE tp2;"
...
docker exec $2 mongoimport -d tp2 --authenticationDatabase admin --username admin --password admin -c producto --file  /migration/productos.json 
...
docker exec $2 mongoimport -d tp2 --authenticationDatabase admin --username admin --password admin -c cliente --file  /migration/cliente.json 
...
docker exec $2 mongoimport -d tp2 --authenticationDatabase admin --username admin --password admin -c factura --file  /migration/factura.json 
...
docker exec $2 mongoimport -d tp2 --authenticationDatabase admin --username admin --password admin -c telefono --file  /migration/telefono.json 
